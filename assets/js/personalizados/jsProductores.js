	...
	dtCRUD = $('#dtCRUD').DataTable({
		...
		,createdRow: function ( row, data, index ) {
			...
			.on('click', '.btnEliminar', function(e){
				e.preventDefault();
				alertify.confirm('¿Está seguro de eliminar el productor?', function(){
					$.ajax({
						type: 'POST',
						data: {
							id: data['ProductorId']
						},
						url: 'Productores/Eliminar',
...
$('#frmCrear').on('submit', function(e){
	...
	$.ajax({
			...
			url: 'Productores/Crear',
			success:function(res){
				switch(res){
					case '1':
						alertify.alert('Advertencia', 'Productor actualizado éxitosamente', function(){
							location.reload();
						});
					break;
					case '2':
						alertify.alert('Advertencia', 'Productor creado éxitosamente', function(){
							location.reload();
						});
					break;
					default:
